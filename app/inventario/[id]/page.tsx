...
import { Menu, FileDown, ClipboardList, CheckCircle2, Circle, Settings2, ArrowRight, ExternalLink } from 'lucide-react';
import { AppSidebar } from '@/components/fluir/app-sidebar';
import { Button } from '@/components/fluir/button';
import { Breadcrumb } from '@/components/fluir/breadcrumb';
import { FluirLogo } from '@/components/fluir/logo';

const MOCK_CAMPAIGN = { id: 'cp-1', name: 'Mapeamento 2024.2' };
      ...
      <main className="flex-1 min-w-0 md:overflow-y-auto print:overflow-visible">
        ...
        <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <Breadcrumb
              items={[
                { label: 'Inventário + Plano', onClick: () => router.push('/inventario') },
                { label: MOCK_CAMPAIGN.name },
              ]}
            />
          </div>
          <div className="flex items-center gap-4">
            <button
              onClick={() => router.push(`/campanhas/${MOCK_CAMPAIGN.id}`)}
              className="hidden sm:inline-flex items-center gap-1.5 text-xs font-medium text-[var(--neutral-500)] hover:text-[var(--navy-700)] transition-colors"
            >
              Ver campanha
          ...
          <div className="max-w-4xl mx-auto mt-6 px-4 print:hidden">
            <div className="bg-white rounded-2xl border border-[var(--neutral-200)] shadow-sm overflow-hidden">
              <div className="flex items-center gap-3 px-6 py-4 border-b border-[var(--neutral-100)]">
                ...
                <Button variant="primary" size="sm" className="gap-1.5 shrink-0" onClick={() => router.push(`/campanhas/${MOCK_CAMPAIGN.id}/inventario`)}>
                  Preencher
